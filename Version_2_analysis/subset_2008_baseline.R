...
subset_2008_baseline = function (entire_dataset) {
  ...
  HRS2012_data_subset  = HRS2012_data %>% filter(HHIDPN %in%  c(HRS2008_data$HHIDPN))
  
  HRS2014_data_subset  = HRS2014_data %>% filter(HHIDPN %in%  c(HRS2008_data$HHIDPN))
  
  HRS2016_data_subset  = HRS2016_data %>% filter(HHIDPN %in%  c(HRS2008_data$HHIDPN))
  
  HRS2018_data_subset  = HRS2018_data %>% filter(HHIDPN %in%  c(HRS2008_data$HHIDPN))
  ...
  ###### add binary exposure and binary outcome 
  #0.no
  #1.yes medical record
  #3.disp  record and has cond: 8 people 
  #4.disp   record and no cond 
  #.d=DK  ~ 0 
  #.r=RF
  # when 1 ~ 1 and 3 ~ 1 then there are 9533 events of diabetes onset in the entire sample
  # when 1 ~ 0, and 3 ~ 1 then there is 8 cases 
  
  WCE_dataset_subset$diabetes_new_bin = case_when(WCE_dataset_subset$diabetes_new == 1 ~ 1, 
                                                  WCE_dataset_subset$diabetes_new == 0 ~ 0, 
                                                  WCE_dataset_subset$diabetes_new == 3 ~ 1, #when this is changes to zero then....ADD 9525 
