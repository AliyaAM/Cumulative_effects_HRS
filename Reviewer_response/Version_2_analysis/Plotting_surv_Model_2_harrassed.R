...
unique(cumulative_effects_dat$discrim_harassed_bin_cat)

unique(cumulative_effects_dat$discrim_harassed_bin_cat)


cumulative_effects_dat$discrim_harassed_bin = cumulative_effects_dat$discrim_bin
#1 = 2 year 
#2 = 4 years 
#3 = 6 years 
...
fit <- coxph(Surv(follow_up, diabetes_new_bin)~ discrim_harassed_bin + continious_age + wealth_noIRA  +  assessed_BMI + hypertension_new_bin + sex_1_2, data = cumulative_effects_dat)
summary_all = summary(fit)

# coeffcients for discrim_harassed_bin: 
...
fit_female <- coxph(Surv(follow_up, diabetes_new_bin)~ discrim_harassed_bin + continious_age + wealth_noIRA  +  assessed_BMI + hypertension_new_bin , data = data_female)
...
fit_male <- coxph(Surv(follow_up, diabetes_new_bin)~ discrim_harassed_bin + continious_age + wealth_noIRA  +  assessed_BMI + hypertension_new_bin , data = data_male)
...
fit_race <- coxph(Surv(follow_up, diabetes_new_bin)~ discrim_harassed_bin + continious_age + wealth_noIRA  +  assessed_BMI + hypertension_new_bin + sex_1_2, data = data_race)
...
fit_BMI <- coxph(Surv(follow_up, diabetes_new_bin)~ discrim_harassed_bin + continious_age + wealth_noIRA  +  assessed_BMI + hypertension_new_bin + sex_1_2, data = data_BMI)
...
write.csv(Model_2_non_interact_results, paste(DATA_ROOT, "/Model_2_discrim_harassed_bin_results.csv", sep = ""))
